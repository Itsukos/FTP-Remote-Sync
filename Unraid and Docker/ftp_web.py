...
<div class="br-layout">
  ...
  <div style="display:flex;flex-direction:column;gap:0">
    ...
    <div class="br-files" id="br-files-pane" style="max-height:360px;overflow-y:auto;position:relative">
      <table>
        <thead><tr>
      ...
      <div id="br-drop-overlay" style="display:none;position:absolute;inset:0;background:rgba(137,180,250,.15);border:2px dashed #89b4fa;border-radius:6px;z-index:10;pointer-events:none;align-items:center;justify-content:center">
        <span style="color:#89b4fa;font-size:16px;font-weight:700">⬆ Drop to upload</span>
      </div>
    </div>
...
// ── Drag-and-drop upload wiring (runs once on page load) ───────────────────
(function(){
  function getPane(){ return document.getElementById("br-files-pane"); }
  function getOverlay(){ return document.getElementById("br-drop-overlay"); }
  let dragCounter = 0;
  ...
  document.addEventListener("dragleave", function(e){
    dragCounter--;
    if(dragCounter<=0){ dragCounter=0; const ov=getOverlay(); if(ov) ov.style.display="none"; }
  });
  ...
  document.addEventListener("drop", function(e){
    dragCounter=0;
    const ov=getOverlay(); if(ov) ov.style.display="none";
    const pane=getPane();
    if(!pane) return;
    const rect=pane.getBoundingClientRect();
    const inPane=(e.clientX>=rect.left&&e.clientX<=rect.right&&e.clientY>=rect.top&&e.clientY<=rect.bottom);
    if(!inPane) return;
    e.preventDefault();
    if(!BR.connected){alert("Connect to a server first.");return;}
    if(e.dataTransfer.files && e.dataTransfer.files.length>0){
      brDropUpload(e.dataTransfer.files);
    }
  });
})();
...
function brConnect(){
  ...
  fetch("/api/br/connect",{method:"POST"})
    .then(r=>r.json()).then(d=>{
      if(d.ok){ BR.connected=true; brSetStatus("Connected to "+d.host,true); brNavigate("/"); brLoadTree("/"); }
      else     { BR.connected=false; brSetStatus("Failed: "+d.error,false); }
    }).catch(e=>{ BR.connected=false; brSetStatus("Error: "+e,false); });
}
...
function brSaveSession(){
  ...
  if(!name) return;
  // Trigger a file download so the user can save the session wherever they want
  fetch("/api/br/save_session",{method:"POST",headers:{"Content-Type":"application/json"},
    ...
    const safe=name.replace(/[^a-zA-Z0-9 _-]/g,"").trim()||"session";
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");
    a.href=url; a.download=safe+".session.json"; a.click();
    URL.revokeObjectURL(url);
  }).catch(e=>alert("Failed: "+e));
}
function brDropUpload(files){
  if(!BR.connected){alert("Connect to a server first.");return;}
  [...files].forEach(file=>{
    const id=BR.qNextId++;
    brAddQueueRow(id,"↑",file.name);
...
def api_br_save_session():
    """
    Return the session JSON as a file download so the browser saves it
    wherever the user chooses — no server-side path needed.
    Also includes password_enc from the active config so --session auto-connect works.
    """
    ...
    payload = _json.dumps(session_data, indent=2).encode("utf-8")
    return Response(
        payload,
        mimetype="application/json",
        headers={
            "Content-Disposition": f'attachment; filename="{safe_name}.session.json"',
